...
def get_token() -> str | None:
    result = session.get("https://cn.pornhub.com")
    # regex extract
...
def performer_by_name(name: str) -> list[PerformerSearchResult]:
    ...
    response = session.get("https://cn.pornhub.com/api/v1/video/search_autocomplete", params={
        "q": name,
        "token": token,
        "pornstars": "true",
        "alt": 0 # hardcoded
    ...
    for model in data.get("models", []):
        results.append({
            "name": model.get("name"),
            "url": f"https://cn.pornhub.com/model/{model.get('slug')}"
        })
    for pornstar in data.get("pornstars", []):
        results.append({
            "name": pornstar.get("name"),
            "url": f"https://cn.pornhub.com/pornstar/{pornstar.get('slug')}"
        })
